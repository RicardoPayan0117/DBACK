...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!is_numeric($costo)) {
        ...
        $sql = "INSERT INTO solicitudes (
            nombre_completo, telefono, email, ubicacion, tipo_vehiculo, marca_vehiculo, modelo_vehiculo, 
            ...
            '$nombre_completo', '$telefono', '$email', '$ubicacion_final', '$tipo_vehiculo', '$marca_vehiculo', '$modelo_vehiculo',
...
<script>
    ...
    function reverseGeocode(lat, lon) {
        // Guardar coordenadas de destino cuando se usa reverse geocoding
        const dlat = document.getElementById('destino_lat');
        const dlng = document.getElementById('destino_lng');
        if (dlat && dlng) {
            dlat.value = lat;
            dlng.value = lon;
        }
    ...
    document.getElementById('obtenerUbicacionDestino').addEventListener('click', () => {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(pos => {
                ...
                // Guardar coordenadas destino si es click de destino
                const dlat = document.getElementById('destino_lat');
                const dlng = document.getElementById('destino_lng');
                if (dlat && dlng) {
                    dlat.value = lat;
    ...
    document.getElementById('obtenerUbicacionOrigen').addEventListener('click', () => {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(pos => {
                ...
                const olat = document.getElementById('origen_lat');
                const olng = document.getElementById('origen_lng');
                if (olat && olng) {
                    olat.value = lat;
                    olng.value = lon;
                }
    ...
    map.on('click', e => {
        ...
        const dlng = document.getElementById('destino_lng');
        if (dlat && dlng) {
            dlat.value = e.latlng.lat;
            dlng.value = e.latlng.lng;
        }
    });
...
<script src="https://unpkg.com/leaflet/dist/leaflet.js"></script>
        ...
        function calcularDistancia() {
            ...
            setTimeout(() => {
                ...
                if (tieneCoords) {
                    ...
                    // Fallback simulado si no hay coords
                    distanciaKm = Math.random() * 50 + 5;
                }
    ...
    mapOrigen.on('click', e => {
        ...
        document.getElementById('origen_lat').value = e.latlng.lat;
        document.getElementById('origen_lng').value = e.latlng.lng;
    });

    markerOrigen.on('dragend', () => {
        const pos = markerOrigen.getLatLng();
        document.getElementById('origen_lat').value = pos.lat;
        document.getElementById('origen_lng').value = pos.lng;
    });
    ...
    // Función para iniciar el pago con PayPal
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('obtenerUbicacionOrigen').addEventListener('click', function() {
                ...
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(pos => {
                        document.getElementById('origen_lat').value = pos.coords.latitude;
                        document.getElementById('origen_lng').value = pos.coords.longitude;
                        calcularDistancia();
                    });
                }
            });
